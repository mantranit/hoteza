  ...
  $(HotezaTV).one("final", function () {
    ...
    if (isset("config.tv.mouse_control")) {
      ...
      document.addEventListener("click", function (e) {
        ...
        var back_arr = [
          "tv_fullscreen_btn_back",
          "tv_mosaic_btn_back",
          "tv_mosaic_preview_btn_back",
        ];
...
function _remove_listener_TV(plugin) {
  var ctx = Object.assign(ctx_channel, {
    ...
    eventListener:
      tv_channellist_type === "mosaic" ? tv_mosaic.event_listener : null,
  });
  ...
  if (channel.background) {
    if (!$id("tv_channel_background")) {
      $(document.body).prepend(
        '<iframe id="tv_channel_background" style="position:absolute;top:0px;left:0px;width:' +
          ww +
          "px;height:" +
          wh +
          'px;border:0;"></iframe>'
...
function _tv_get_network_info() {
  ...
  d.resolve({
    ip: "127.0.0.1",
    mac: storage.getItem("desktop_mac"),
    type: "UNKNOWN",
  });
  return d.promise();
}
...
function __tv_set_window_size() {
  ...
  // } else {
  zoom = Math.min(
    winHeight < wh ? winHeight / wh : 1,
    winWidth < ww ? winWidth / ww : 1
  );
  d.resolve("hd720");
  // }
...
var _tv_get_info = {
  ...
  _parse: function () {
    if (window.bowser) {
      ...
      this._model =
        this._browser.os.name +
        " " +
        (this._browser.os.versionName || this._browser.os.version || "");
      this._firmware =
        this._browser.browser.name + " " + this._browser.browser.version;
    }
  },
  _model: "PC",
  _firmware: "Browser",
