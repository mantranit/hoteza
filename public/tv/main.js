log.add(
  "APP: version " + version.v + " (" + version.date + ") build: " + version.git
);
...
function tv_size_init() {
  ...
  document.title = "Seashells Phu Quoc Hotel & Spa (" + ww + "x" + wh + ")";
}
...
function js_error(e, url, line, column, error) {
  ...
  log.add(
    "Global error: " + error_message + ", url: " + url + ", line: " + line
  );
...
function document_ready() {
  ...
  if (get_language() !== "en") {
    var localeLangForMoment =
      get_language() === "si" ? "sl" : get_language().toLowerCase();

    $(document.head).append(
      '<script type="text/javascript" src="j/libs/moment/locale/' +
        localeLangForMoment +
        '.js"></script>'
    );
  }
  ...
  if (isset("config.tv.roomcontrol")) {
    ...
    $.cachedScript("j/rcu.js?v=" + version.v).done(function () {
      build_rcu();
    });
  }
...
function filter_content_by_group() {
  ...
  log.add(
    "FILTER: Filtered items by group: +" +
      tmp_i +
      "|-" +
      tmp_i2 +
      "|" +
...
function filter_content_by_device(obj, device) {
  ...
  log.add(
    'FILTER: Filtered items by device "' +
      device +
      '": +' +
      tmp_i2 +
      "/ -" +
...
function filter_content_by_device_for_struct(obj, device) {
  ...
  function _filter(o, _device) {
    for (var key in o) {
      if (
        (o[key].devices && !o[key].devices.length) ||
        !o[key].devices ||
 ...
 * */
function renderPageOnTheStructv2(
  id,
  data_obj,
  tplType,
  klass,
  ...
  if (tplType === "menu_classic" || tplType === "menu_scandic") {
  ...
  return UI.build_page({
    ...
    devices:
      data.devices && data.devices.length ? data.devices.toString() : null,
...
function getObjectLang() {
  if (lang.empty) {
    ...
    lang.now = getlang(
      "mobileAppContent-contentPage-button-hotelServiceRequest-now"
    );
    ...
    lang.cartIsEmpty = getlang(
      "mobileAppContent-mainContent-label-cartIsEmpty"
    );
    ...
    lang.chooselang = getlang(
      "mobileAppContent-mainContent-settings-chooselang"
    );
    ...
    lang.selectrecipient = getlang(
      "mobileAppContent-mainContent-messages-selectrecipient"
    );
    lang.messages = getlang("messages");
    lang.addPcsToCart = getlang(
      "mobileAppContent-mainContent-label-addPcsToCart"
    );
    lang.addToCart = getlang("mobileAppContent-mainContent-label-addToCart");
    lang.continueshopping = getlang(
      "mobileAppContent-mainContent-button-continueshopping"
    );
    lang.gotocart = getlang("mobileAppContent-mainContent-button-gotocart");
    lang.shopordertext = getlang(
      "mobileAppContent-mainContent-label-shopordertext"
    );
    lang.order = getlang("mobileAppContent-mainContent-header-order");

    lang.controlserviceinfo = getlang(
      "mobileAppContent-mainMenuPage-label-controlservicesinfo"
    );
    lang.controlservicesDND = getlang(
      "mobileAppContent-mainMenuPage-label-controlservicesDND"
    );
    lang.controlservicescleanroom = getlang(
      "mobileAppContent-mainMenuPage-label-controlservicescleanroom"
    );
    lang.temperaturecontrol = getlang(
      "mobileAppContent-mainMenuPage-label-temperaturecontrol"
    );
    lang.curtainscontrol = getlang(
      "mobileAppContent-mainMenuPage-label-curtainscontrol"
    );
    lang.curtainsopen = getlang(
      "mobileAppContent-mainMenuPage-label-curtainsopen"
    );
    lang.lightcontrol = getlang(
      "mobileAppContent-mainMenuPage-label-lightcontrol"
    );
    ...
    lang.roomcontrol = getlang(
      "mobileAppContent-mainMenuPage-label-roomcontrol"
    );
    ...
    lang.channel = getlang(
      "mobileAppContent-mainMenuPage-label-tvcontrol-channel"
    );
    lang.tvcontrol = getlang("mobileAppContent-mainMenuPage-label-tvcontrol");
    lang.translated = getlang(
      "mobileAppContent-mainContent-label-translatedMessage"
    );
