...
function salinda_resort_start() {
  ...
  var importStyle = (function (oHead) {
    function loadError(oError) {
      throw new URIError(
        "The style " + oError.target.src + " is not accessible."
      );
    }
    ...
    return function (sSrc, sId, fOnload, fOnerror) {
      ...
      if (fOnload) {
        oStyle.onload = fOnload;
      }
      if (fOnerror) {
        oStyle.onerror = fOnerror;
      }
  ...
  importScript("j/libs/jquery-2.2.4.min.js", function () {
  ...
  function importAll() {
    ...
    importStyle(
      ...
      function () {
        importStyle(
          "tv/config.css?_=" + Math.random(),
          "customization_css_old"
        );
      }
    );
  }
