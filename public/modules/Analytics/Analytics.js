...
window.Analytics = {
	deps: [],
	url: 'http' + (isset('config.use_ssl')?'s':'') + '://analytics.hoteza.com/api/',
	observer: null,
	timer: {
		channels: null,
		pages: null,
	...
	send: function (type) {
		...
		if(data.length){
			$.post(
				Analytics.url + type + "/" + get_hotelId(),
				{ data: data }
			).done(function () {
	...
	sendData: function () {
		...
		Analytics.timer.sendingData = setTimeout(Analytics.sendData, Analytics.sendingDelay);
	},
	...
	collector: function () {
		return 'MutationObserver' in window ?
			collectorForMutationObserver : collectorForAnimation;
		...
		function collectorForMutationObserver(mutations) {
			for (var i = 0; i < mutations.length; i++) {
				...
				Analytics.timer['channels'] = setTimeout(Analytics.statChannels.bind(null, 'channels'), Analytics.duration);
			}
	...
	statChannels: checkEnv(function (id, duration) {
		...
		duration = typeof duration !== 'undefined' ? duration : Analytics.duration;
	...
	hitsNotifications: function (payload) {
		...
		var notice = stat[payload.target] || { show: 0, goal: 0, close: 0, timeout: 0 };
