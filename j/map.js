	...
	if($('#guide_map').length){
		if(check_internet()){
			if(!$('#google_maps_script').length){
				$('#guide_map').html('<div style="text-align:center;padding-top:20px;">Loading Google Maps</div>');
				//Включение Google карт
				var script = document.createElement('script');
				script.id = "google_maps_script";
				script.type = "text/javascript";
				if(only_js){
					script.src = "https://maps.googleapis.com/maps/api/js?key="+googleMapsAPIKey+"&callback=map_preinitialize&language="+(get_language());
				}else{
					script.src = "https://maps.googleapis.com/maps/api/js?key="+googleMapsAPIKey+"&callback=map_initialize&language="+(get_language());
				}
			...
			$('#guide_map').html('<div style="text-align:center;padding-top:20px;">Internet Connection required<br/><div class="button" onvclick="map_init()">Retry</div></div>');
		}
	}
}
...
function map_initialize(){
	...
	var mapOptions = {
		zoom: 14,
		maxZoom: 16,
		center: new google.maps.LatLng(structv2.guide.hotel_position.lat,structv2.guide.hotel_position.lng),
	...
	guide_map = new google.maps.Map(document.getElementById("guide_map"), mapOptions);
	//map_fix();
			...
			if(typeof(guide) == 'object'){
				if(typeof(guide[group]) == 'object'){
					...
					if(guide[group].length){
						...
						latlngbounds.extend(new google.maps.LatLng(structv2.guide.hotel_position.lat, structv2.guide.hotel_position.lng));
						...
						$(guide[group]).each(function(){
							...
							var link = '';
							if(this[4]){
								link = '<p><a class="map_info_sign" onclick="navigate(\''+this[4]+'\');">More information</a></p>';
							}
							google.maps.event.addListener(marker, 'click', function(){
								...
								var route = '';
								if(false){
									route = '<a onclick="map_show_route(new google.maps.LatLng'+this.getPosition()+');">route</a>';
								}

								infowindow = new google.maps.InfoWindow({
									pixelOffset: new google.maps.Size(-1,10),
									content: '<div class="guide_infowindow"><span>'+title+'</span><p>'+descr+'</p>'+route+link+'</div>',
									maxWidth: 220
								});
...
function map_show_me(cb){
	if(typeof(google) == 'undefined'){
		...
		if(!guide_map){
			...
			setTimeout(function(){
				if(!cb){
					navigator.geolocation.getCurrentPosition(_map_show_me, map_error, {timeout: 10000, enableHighAccuracy:true});
				}else{
					navigator.geolocation.getCurrentPosition(
						function(coords){
							_map_show_me(coords);
							cb();
						}, map_error, {timeout: 10000, enableHighAccuracy: true}
					);
				}
			}, 300);
		}
...
function _map_show_me(geopos){
	...
	var tmp_pos = new google.maps.LatLng(geopos.coords.latitude,geopos.coords.longitude);
...
function map_show_hotel(){
	if(typeof(google) == 'undefined'){
		...
		if(!guide_map){
			...
			var tmp_pos = new google.maps.LatLng(structv2.guide.hotel_position.lat,structv2.guide.hotel_position.lng);
			if(!map_hotel){
				...
				google.maps.event.addListener(map_hotel, 'click', function(){
					...
					infowindow = new google.maps.InfoWindow({
						pixelOffset: new google.maps.Size(-1,10),
						content: '<div class="guide_infowindow"><span>'+structv2.guide.hotel_name+'</span><p></p>'+route+'</div>',
						maxWidth: 220
					});
...
//		console.log(coords);
		...
		map_show_me(function(){map_show_route(coords);});
	}
}
